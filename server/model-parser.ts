...
export class ModelParser {
  ...
  async parseAndImport(): Promise<{
    ...
    try {
      ...
      const dataArray = XLSX.utils.sheet_to_json(dataSheet, {
        header: 1,
        defval: null,
      }) as any[][];
      ...
      console.log(
        "Найдены колонки:",
        Object.keys(columnMap).slice(0, 20).join(", "),
      );
  ...
  private parseCtpRow(
    ...
    const commentSKIPiA = this.cleanString(
      row[columnMap["Комментарий СКИПиА"]],
    );
    ...
    const normativMinenergo = this.parseNumber(
      row[columnMap["Норматив подпитки Минэнерго 0,25% Vсети"]],
    );
  ...
  private async importToDatabase(parsedCTPs: ParsedCTP[]): Promise<{
    ...
    for (const districtName of Array.from(uniqueDistricts)) {
      const existing = await db.districts.findFirst({
    ...
    for (const vyvodName of Array.from(uniqueVyvods)) {
      ...
      const existing = await db.vyvod.findUnique({
        where: { code: vyvodCode },
      });
    ...
    for (let i = 0; i < parsedCTPs.length; i += batchSize) {
      ...
      for (const parsedCTP of batch) {
        try {
          ...
          const existing = await db.cTP.findUnique({
            where: { code: parsedCTP.code },
          });
          ...
          const ctpData: any = {
            ...
            districtId: parsedCTP.districtName
              ? districtMap.get(parsedCTP.districtName)
              : null,
            vyvodId: parsedCTP.vyvodName
              ? vyvodMap.get(parsedCTP.vyvodName)
              : null,
          ...
          if (parsedCTP.measurements.length > 0) {
            ...
            for (
              let j = 0;
              j < parsedCTP.measurements.length;
              j += measurementBatchSize
            ) {
              const measurementBatch = parsedCTP.measurements.slice(
                j,
                j + measurementBatchSize,
              );
      ...
      console.log(
        `Импортировано ${Math.min(i + batchSize, parsedCTPs.length)}/${parsedCTPs.length} ЦТП...`,
      );
    }

    console.log(
      `✅ Импорт завершен: ${ctpCount} ЦТП, ${measurementCount} измерений, ${vyvodMap.size} выводов`,
    );
