...
export interface IStorage {
  ...
  updateStatisticalParams(
    params: InsertStatisticalParams,
  ): Promise<StatisticalParams>;
  calculateControlBoundaries(
    ctpId: string,
...
export class MemStorage implements IStorage {
  ...
  private initializeTestData(): void {
    ...
    [rts1, rts2, rts3, rts4, rts5].forEach((rts) =>
      this.rtsData.set(rts.id, rts),
    );
    ...
    districts.forEach((district) =>
      this.districtsData.set(district.id, district),
    );
  ...
  async getRTSWithStats(): Promise<RTSWithStats[]> {
    ...
    for (const rts of rtsList) {
      const ctpList = Array.from(this.ctpData.values()).filter(
        (ctp) => ctp.rtsId === rts.id,
      );
  ...
  async getDistrictsByRTS(rtsId: string): Promise<District[]> {
    return Array.from(this.districtsData.values()).filter(
      (district) => district.rtsId === rtsId,
    );
  }
  ...
  async getMeasurements(
    ...
    let measurements = Array.from(this.measurementsData.values()).filter(
      (measurement) => measurement.ctpId === ctpId,
    );
  ...
  async getStatisticalParams(
    ...
    return Array.from(this.statisticalParamsData.values()).find(
      (params) => params.ctpId === ctpId,
    );
  }
  ...
  async updateStatisticalParams(
    ...
    const existing = Array.from(this.statisticalParamsData.values()).find(
      (p) => p.ctpId === params.ctpId,
    );
    ...
    if (existing) {
      ...
      const id = randomUUID();
      const newParams: StatisticalParams = {
        ...params,
        id,
  ...
  async getRecommendations(filters?: {
    ...
    if (filters?.ctpId) {
      recommendations = recommendations.filter(
        (rec) => rec.ctpId === filters.ctpId,
      );
    }
    if (filters?.type) {
      recommendations = recommendations.filter(
        (rec) => rec.type === filters.type,
      );
    }
    if (filters?.priority) {
      recommendations = recommendations.filter(
  ...
  async updateRecommendationStatus(id: string, status: string): Promise<void> {
    ...
    if (recommendation) {
      this.recommendationsData.set(id, {
        ...recommendation,
        status,
        updatedAt: new Date(),
      });
  ...
  async getUploadHistory(): Promise<UploadedFile[]> {
    return Array.from(this.uploadedFilesData.values()).sort((a, b) => {
