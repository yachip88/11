...
export class StatisticalAnalysis {
   ...
   */
  static calculateControlBoundaries(
    data: number[],
    confidence: number = 3,
  ): StatisticalResult {
    if (data.length < 10) {
      throw new Error(
        "Недостаточно данных для расчета контрольных границ (минимум 10 точек)",
      );
    }
  ...
  static detectPatterns(
    ...
    if (maxConsecutiveAbove >= 7) {
      patterns.push(
        `${maxConsecutiveAbove} последовательных точек выше центральной линии`,
      );
    }
    if (maxConsecutiveBelow >= 7) {
      patterns.push(
        `${maxConsecutiveBelow} последовательных точек ниже центральной линии`,
      );
    }
  ...
  static calculateProcessCapability(
    ...
    const cp =
      (specLimits.upperSpec - specLimits.lowerSpec) /
      (6 * stats.standardDeviation);

    const cpkUpper =
      (specLimits.upperSpec - stats.mean) / (3 * stats.standardDeviation);
    const cpkLower =
      (stats.mean - specLimits.lowerSpec) / (3 * stats.standardDeviation);
